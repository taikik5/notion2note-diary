"""
Main orchestrator for Notion-to-Note AutoDrafter.

This script:
1. Fetches articles with Status='Ready' from Notion
2. Formats each article using OpenAI API
3. Posts articles as drafts to note.com
4. Updates Notion status to 'Done'
"""
...
def main() -> int:
    """Main entry point."""
    print("=" * 50)
    print("Notion-to-Note AutoDrafter")
    print("=" * 50)
    ...
    for i, article in enumerate(articles, 1):
        ...
        try:
            ...
            memo_content = article["memo_content"]

            if not memo_content.strip():
                print("Warning: Empty memo content, skipping.")
                error_count += 1
                continue

            title, body = format_article(memo_content, article["date"])
            ...
            print("\n[Step 4] Updating Notion status to 'Done'...")
